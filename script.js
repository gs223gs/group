document.addEventListener('DOMContentLoaded', () => {
  ...
  createGroupsBtn.addEventListener('click', () => {
      ...
      const groups = createBalancedRandomGroups(currentData, groupCount);
      if (!groups) {
          return;
      }
  ...
  function createBalancedRandomGroups(data, groupCount) {
      ...
      // グループに女性を均等に分配
      const femalesPerGroup = distributeFemales(females, groupCount, maxFemalesPerGroup);
      if (femalesPerGroup === null) {
          alert('性別バランスを保つことができませんでした。グループ数や参加者の性別を再確認してください。');
          return null;
      }
      ...
      // 残りの男性を均等に割り当て
      const remainingMales = males.slice(0, males.length);
  ...
  function distributeFemales(females, groupCount, maxFemalesPerGroup) {
      ...
      if (femaleIndex < females.length) {
          // 残った女性を他のグループに追加
          for (let i = 0; i < females.length - femaleIndex; i++) {
              const groupIndex = i % groupCount;
              femalesPerGroup[groupIndex].push(females[femaleIndex++]);
          }
      }

      // 再度、各グループの女性数が最大許容数を超えていないか確認
      for (let i = 0; i < groupCount; i++) {
          const femaleCount = femalesPerGroup[i].length;
          if (femaleCount > maxFemalesPerGroup[i]) {
              return null;
          }
      }

      return femalesPerGroup;
  }
